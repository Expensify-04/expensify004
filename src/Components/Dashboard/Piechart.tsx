...
const Piechart: React.FC = () => {
  ...
  const handleAddExpense = (personName: string, expense: SingleExpense, limit: number) => {
    setPeopleExpenses((prev) => {
      const person = prev.find((p) => p.personName.toLowerCase() === personName.toLowerCase());

      const totalSpent = person ? person.expenses.reduce((sum, e) => sum + e.amount, 0) : 0;
    ...
    <div className="flex flex-col w-full">
      <div className="flex flex-col items-center min-h-screen px-4 py-8 pt-24">
        ...
        <button
          ...
          className="px-6 py-2 mb-6 text-white rounded bg-cyan-600 hover:bg-cyan-700">
          ➕ Add Expense
        </button>
        ...
        <div className="grid grid-cols-1 gap-8 px-10 mt-10 sm:grid-cols-2 lg:grid-cols-3">
          {peopleExpenses.map((person, index) => {
            const totalSpent = person.expenses.reduce((sum, e) => sum + e.amount, 0);
